...
function Cart() {
  ...
  const checkout = () => {
      ...
      if (stock[0].stock - data.quantity >= 0) {
        ...
        dispatch(
          updatestock(
            ...
            state.stock,
            item_stock[0],
            () => state.product.findIndex((x) => x.id === data.productId),
            data.quantity
          )
        );
      }
    ...
    let othercart = JSON.parse(localStorage.getItem("cart")).filter(
      (product) =>
        product.userId !== JSON.parse(localStorage.getItem("login")).id
    );
        ...
        !state.loading ? (
          state.cart.length !== 0 ? (
            ...
            >
              ...
              <p style={{ color: "#7B7B7B" }}>Cart is empty!</p>
            </Stack>
          )
        ) : (
          ""
