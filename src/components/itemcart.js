                    ...
                    <Stack className="h-100 justify-content-between" gap={3}>
                        ...
                        <Stack className='flex-grow-0 align-items-center' direction="horizontal">
                            <InputGroup className="w-50" size="sm">
                                ...
                                {
                                    state.stock.filter(x => x.id === product.id)[0].stock - quantity <= 0
                                        ? <InputGroup.Text role="button">max</InputGroup.Text>
                        ...
                        </Stack>
                        {
                            state.stock.filter(x => x.id === product.id)[0].stock - quantity <= 0
                                ? state.stock.filter(x => x.id === product.id)[0].stock - quantity === 0
                                    ? <p style={{ color: 'red' }}>maximum stock</p>
                                    : <p style={{ color: 'red' }}>insufficient stock</p>
                                : ''
                        }
                    </Stack>
                </Col>
